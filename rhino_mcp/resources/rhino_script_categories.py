...
def get_all_functions(category=None):
    """
    Return all RhinoScriptSyntax functions (full Rhino 7 API roster).
    
    Args:
        category (str, optional): If set, only return functions in this category
            (e.g. "curve", "surface", "mesh", "object", "layer").
    
    Returns:
        list: [{"function_name": str, "category": str}, ...]
    """
    if category is None:
        return [{"function_name": k, "category": v} for k, v in FUNCTION_CATEGORY_MAP.items()]
    cat_lower = category.lower()
    return [{"function_name": k, "category": v} for k, v in FUNCTION_CATEGORY_MAP.items() if v.lower() == cat_lower]


def get_categories():
    """Return sorted list of unique RhinoScriptSyntax category names."""
    return sorted(set(FUNCTION_CATEGORY_MAP.values()))
