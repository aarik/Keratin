...
async def server_lifespan(server: FastMCP) -> AsyncIterator[Dict[str, Any]]:
    ...
    Establishes the Rhino connection used by all tools (get_rhino_connection().send_command).
    Start the Rhino script (rhino_script.py) and load the script in Rhino before connecting."""
    rhino_conn = None

    try:
        logger.info("RhinoMCP server starting up")

        # Connect to Rhino script socket (localhost:9876); all MCP tools use this connection
        try:
